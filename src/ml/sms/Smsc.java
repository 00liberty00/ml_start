...
public class Smsc {
    ...
    String SMSC_LOGIN = "rentcrimea";     // логин клиента
    String SMSC_PASSWORD = "vfksirf";  // пароль или MD5-хеш пароля в нижнем регистре
     ...
     * @param time - необходимое время доставки в виде строки (DDMMYYhhmm,
     * h1-h2, 0ts, +m)
     * @param id - идентификатор сообщения. Представляет собой 32-битное число в
     * диапазоне от 1 до 2147483647.
     * @param format - формат сообщения (0 - обычное sms, 1 - flash-sms, 2 -
     * wap-push, 3 - hlr, 4 - bin, 5 - bin-hex, 6 - ping-sms)
     * @param sender - имя отправителя (Sender ID). Для отключения Sender ID по
     * умолчанию необходимо в качестве имени передать пустую строку или точку.
     * @param query - строка дополнительных параметров, добавляемая в URL-запрос
     * ("valid=01:00&maxsms=3&tz=2")
     * @return array (<id>, <количество sms>, <стоимость>, <баланс>) в случае
     * успешной отправки или массив (<id>, -<код ошибки>) в случае ошибки
     */
    public String[] send_sms(String phones, String message, Integer translit, String time, String id, Integer format, String sender, String query) {
     ...
     * @return array для отправленного SMS (<статус>, <время изменения>,
     * <код ошибки sms>) для HLR-запроса (<статус>, <время изменения>,
     * <код ошибки sms>, <код страны регистрации>, <код оператора абонента>,
     * <название страны регистрации>, <название оператора абонента>,
     * <название роуминговой страны>, <название роумингового оператор
     * <код IMSI SIM-карты>, <номер сервис-центра>) либо array(0, -<код ошибки>)
     * в случае ошибки
     */
    ...
    private String[] _smsc_send_cmd(String cmd, String arg) {
        ...
        try {
            String url = (SMSC_HTTPS ? "https" : "http") + "://smsc.ua/sys/" + cmd + ".php?login=" + URLEncoder.encode(SMSC_LOGIN, SMSC_CHARSET)
            ...
            do {
                ...
                if (i == 2) {
                    url = url.replace("://smsc.ua/", "://www2.smsc.ua/");
                }
